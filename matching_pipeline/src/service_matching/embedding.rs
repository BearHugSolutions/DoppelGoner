...
use crate::db::PgPool;
use crate::models::{ServiceId, MatchMethodType, MatchValues, ServiceEmbeddingMatchValue};
...
pub async fn find_matches(
    ...
    let rows = conn.query(
        "SELECT id, name, embedding_v2 FROM public.service 
         ...
         ORDER BY id",
        &[],
    ).await.context("Failed to query services with embeddings")?;
    ...
    info!("Found {} services with embeddings to process", services_count);
    ...
    info!("Processed {} service embeddings for comparison", service_embeddings.len());

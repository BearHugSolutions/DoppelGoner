...
use crate::db::PgPool;
use crate::models::{ServiceId, MatchMethodType, MatchValues, EmailMatchValue};
...
pub async fn find_matches(
    ...
    let rows = conn.query(
        "SELECT id, email FROM public.service 
         ...
         ORDER BY id",
        &[],
    ).await.context("Failed to query services with emails")?;
    ...
    for row in rows {
        ...
        if !normalized_email.is_empty() {
            email_map.entry(normalized_email.clone())
                .or_default()
                .push((ServiceId(service_id), email));
            total_services += 1;
        }
    }
    info!("Built email map with {} unique normalized emails", email_map.len());

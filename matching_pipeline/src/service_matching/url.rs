...
use crate::db::PgPool;
use crate::models::{ServiceId, MatchMethodType, MatchValues, ServiceUrlMatchValue};
...
pub async fn find_matches(
    ...
    let rows = conn.query(
        "SELECT id, url FROM public.service 
         ...
         ORDER BY id",
        &[],
    ).await.context("Failed to query services with URLs")?;
    ...
    for (domain, services) in domain_map {
        ...
        for i in 0..services.len() {
            for j in (i+1)..services.len() {
                ...
                let pre_rl_confidence = calculate_url_confidence(original_url1, original_url2, &domain, matching_slug_count);

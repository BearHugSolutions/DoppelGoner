...
use chrono::{Utc, NaiveDateTime}; // NaiveDateTime for NewSuggestedAction
...
async fn create_entity_group(
    ...
    let mut conn = pool.get().await.context("Email: Failed to get DB connection for entity group creation")?;
    let tx = conn.transaction().await.context("Email: Failed to start transaction for entity group creation")?;
    let new_entity_group_id = EntityGroupId(Uuid::new_v4().to_string());
    let match_values_json = serde_json::to_value(match_values).context("Email: Failed to serialize match values")?;

    let rows_affected = tx.execute(
        INSERT_ENTITY_GROUP_SQL,
        &[
            &new_entity_group_id.0, &entity_id_1.0, &entity_id_2.0,
            &MatchMethodType::Email.as_str(), &match_values_json,
            &final_confidence_score, &pre_rl_confidence_score,
        ],
    ).await.context("Email: Failed to execute insert entity group")?;
    ...
    if rows_affected > 0 {
        ...
        if let (Some(orch_arc), Some(features_vec)) = (reinforcement_orchestrator, &features_for_snapshot) {
            ...
            if let Err(e) = tx.query_one(
                ...
                &[
                    ...
                    &MatchMethodType::Email.as_str(), &pre_rl_confidence_score,
                    &final_confidence_score, &(confidence_tuner_ver as i32),
                ],
            ).await {
                warn!("Email: Failed to log decision snapshot for new group {}: {}", new_entity_group_id.0, e);
            }
        }
        ...
        if final_confidence_score < MODERATE_LOW_SUGGESTION_THRESHOLD {
            ...
            let suggestion = NewSuggestedAction {
                ...
                entity_id: None, group_id_1: Some(new_entity_group_id.0.clone()), group_id_2: None, cluster_id: None,
        ...
        tx.commit().await.context("Email: Failed to commit transaction (group created)")?;
        Ok(true)
    } else {
        tx.commit().await.context("Email: Failed to commit transaction (no new group created)")?;
        Ok(false)
    }
}
...
pub async fn find_matches(
    ...
    let mut initial_read_conn = pool.get().await.context("Email: Failed to get DB connection for initial reads")?;
    let existing_entity_groups: HashSet<(EntityId, EntityId)> =
        fetch_existing_pairs(&*initial_read_conn, MatchMethodType::Email).await?;
    info!("Email: Found {} existing email-matched pairs in entity_group.", existing_entity_groups.len());
    ...
    let email_rows = initial_read_conn.query(email_query, &[]).await.context("Email: Failed to query entities with emails")?;
    ...
    for row in &email_rows {
        ...
        if !normalized.is_empty() {
            email_map.entry(normalized).or_default().insert(entity_id, email);
        }
    }
            ...
            for j in (i + 1)..entities_sharing_email.len() {
                ...
                let (e1_id, e1_orig_email, e2_id, e2_orig_email) =
                    if entity_id1_obj_orig.0 < entity_id2_obj_orig.0 {
                        (entity_id1_obj_orig, original_email1, entity_id2_obj_orig, original_email2)
                    } else {
                        (entity_id2_obj_orig, original_email2, entity_id1_obj_orig, original_email1)
                    };
                ...
                if let Some((sig1_data, sig2_data)) = &current_signatures_opt {
                    match check_comparison_cache(pool, e1_id, e2_id, &sig1_data.signature, &sig2_data.signature, &MatchMethodType::Email).await {
                        Ok(Some(cached_eval)) => {
                            cache_hits_count += 1;
                            debug!("Email: Cache HIT for pair ({}, {}). Result: {}, Score: {:?}", e1_id.0, e2_id.0, cached_eval.comparison_result, cached_eval.similarity_score);
                            processed_pairs_this_run.insert(current_pair_ordered.clone());
                            continue;
                        }
                        Ok(None) => { debug!("Email: Cache MISS for pair ({}, {}). Proceeding.", e1_id.0, e2_id.0); }
                        Err(e) => { warn!("Email: Error checking comparison cache for pair ({}, {}): {}. Proceeding.", e1_id.0, e2_id.0, e); }
                    }
                }

                let mut pre_rl_confidence_score = 1.0;
                if is_generic_organizational_email(&normalized_shared_email) { pre_rl_confidence_score *= 0.9; }
                let email_count = email_frequency.get(&normalized_shared_email).cloned().unwrap_or(1);
                if email_count > 10 { pre_rl_confidence_score *= 0.85; }
                else if email_count > 5 { pre_rl_confidence_score *= 0.92; }
                    ...
                    match if let Some(_cache_service) = feature_cache.as_ref() {
                        ...
                        Ok(features) => {
                            if !features.is_empty() {
                                ...
                                match ro_arc.lock().await.get_tuned_confidence(&MatchMethodType::Email, pre_rl_confidence_score, features_vec_for_rl.as_ref().unwrap()) {
                                    Ok(tuned_score) => final_confidence_score = tuned_score,
                                    Err(e) => warn!("Email: RL tuning failed for ({}, {}): {}.", e1_id.0, e2_id.0, e),
                                }
                            } else { warn!("Email: Extracted features vector is empty for pair ({}, {}).", e1_id.0, e2_id.0); }
                        }
                        Err(e) => warn!("Email: Feature extraction failed for ({}, {}): {}.", e1_id.0, e2_id.0, e),
                    }
                }
                ...
                match create_entity_group(
                    ...
                    pipeline_run_id, &normalized_shared_email,
                    ...
                    Err(e) => {
                        ...
                        warn!("Email: create_entity_group failed for pair ({}, {}): {}", e1_id.0, e2_id.0, e);
                    }
                }
                ...
                if let Some((sig1_data, sig2_data)) = &current_signatures_opt {
                    if let Err(e) = store_in_comparison_cache(
                        ...
                        &MatchMethodType::Email, pipeline_run_id,
                        comparison_outcome_for_cache, Some(final_confidence_score),
                        features_json_for_cache.as_ref(),
                    ).await {
                        warn!("Email: Failed to store in comparison_cache for ({}, {}): {}", e1_id.0, e2_id.0, e);
                    }
                }
    ...
    if individual_operation_errors > 0 {
        warn!("Email: {} errors during individual pair operations.", individual_operation_errors);
    }
...
async fn fetch_existing_pairs(
    ...
    for row in rows {
        ...
        if id1_str < id2_str { existing_pairs.insert((EntityId(id1_str), EntityId(id2_str))); }
        else { existing_pairs.insert((EntityId(id2_str), EntityId(id1_str))); }
    }
    Ok(existing_pairs)
}

fn calculate_email_frequency(email_map: &HashMap<String, HashMap<EntityId, String>>) -> HashMap<String, usize> {
    email_map.iter().map(|(k, v)| (k.clone(), v.len())).collect()
}

pub fn normalize_email(email: &str) -> String {
    let email_trimmed = email.trim().to_lowercase();
    if !email_trimmed.contains('@') { return email_trimmed; }
    let parts: Vec<&str> = email_trimmed.splitn(2, '@').collect();
    if parts.len() != 2 { return email_trimmed; }
    ...
    } else { local_part_no_plus };
    let final_domain_part = match domain_part {
        "googlemail.com" => "gmail.com",
        _ => domain_part,
    };
    if final_local_part.is_empty() { String::new() }
    else { format!("{}@{}", final_local_part, final_domain_part) }
}

fn is_generic_organizational_email(email: &str) -> bool {
    ["info@", "contact@", "office@", "admin@"].iter().any(|prefix| email.starts_with(prefix))
}
...
use crate::config::{CRITICALLY_LOW_SUGGESTION_THRESHOLD, MODERATE_LOW_SUGGESTION_THRESHOLD}; // Ensure these are in config
...
pub async fn find_matches(
    ...
    info!(
        "Starting parallel URL matching (run ID: {}) with INCREMENTAL CHECKS...",
        pipeline_run_id
    );

    let existing_entity_groups_arc = fetch_existing_entity_groups(pool).await?;
    info!(
        "URL: Found {} existing URL-matched pairs in entity_group.",
        existing_entity_groups_arc.len()
    );
    ...
    for chunk_of_domain_entries in
        domain_entries.chunks(DOMAIN_PROCESSING_BATCH_SIZE * MAX_PARALLEL_DOMAINS_IN_FLIGHT)
    {
        ...
        for domain_task_batch in chunk_of_domain_entries.chunks(MAX_PARALLEL_DOMAINS_IN_FLIGHT) {
            for (domain, entities) in domain_task_batch {
                // domain_task_batch is a slice of (String, Vec<EntityUrlInfo>)
    ...
    let avg_confidence = if !final_stats_guard.confidence_scores.is_empty() {
        final_stats_guard.confidence_scores.iter().sum::<f64>()
            / final_stats_guard.confidence_scores.len() as f64
    } else {
        0.0
    };
    ...
    let method_stats = MatchMethodStats {
        ...
        avg_group_size: if final_stats_guard.new_pairs_created_db > 0 {
            final_stats_guard.entities_in_new_pairs.len() as f64
                / final_stats_guard.new_pairs_created_db as f64
        } else {
            0.0
    ...
    info!(
        "URL RL Feature Extraction: {} successful, {} failed.",
        final_stats_guard.rl_feature_extractions, final_stats_guard.rl_feature_failures
    );
...
async fn process_domain_parallel(
    ...
    for i in 0..entities_in_domain.len() {
        for j in (i + 1)..entities_in_domain.len() {
            if pairs_considered_in_domain_count >= MAX_PAIRS_PER_DOMAIN_HARD_LIMIT {
                warn!(
                    "URL: Domain '{}' reached HARD pair limit ({}). Stopping pair generation.",
                    domain_name, MAX_PAIRS_PER_DOMAIN_HARD_LIMIT
                );
                break;
            }
    ...
    if pairs_considered_in_domain_count > MAX_PAIRS_PER_DOMAIN_SOFT_WARN {
        info!(
            "URL: Domain '{}' has {} potential pairs (exceeds soft limit {}).",
    ...
    if let Ok(results) = process_pairs_result {
        ...
        warn!(
            "URL: Error in process_pairs_for_domain_with_caching for domain {}: {}",
            domain_name, e
        );
        let mut stats_guard = shared_stats_arc.lock().await;
        stats_guard.task_errors += 1;
...
async fn process_pairs_for_domain_with_caching(
    ...
    // Returns (domain_cache_hits, domain_rl_feat_extract, domain_rl_feat_fail)
    ...
    struct RlCacheMissInfo {
        entity1_info: EntityUrlInfo,
        entity2_info: EntityUrlInfo, // Keep full info
        ordered_e1_id: EntityId,
        ordered_e2_id: EntityId,
        base_confidence: f64,
        match_values: MatchValues,
        sig1_at_comp: Option<String>,
        sig2_at_comp: Option<String>,
    ...
    for (entity1_info, entity2_info) in potential_pairs {
        ...
        {
            (
                entity1_info.entity_id.clone(),
                entity2_info.entity_id.clone(),
            )
        } else {
            (
                entity2_info.entity_id.clone(),
        ...
        }
        {
            ...
            if processed_set_guard.contains(&current_pair_ordered) {
                continue;
            }
        ...
        let current_signatures_opt =
            match get_current_signatures_for_pair(pool, &e1_id_ordered, &e2_id_ordered).await {
                Ok(sigs) => sigs,
                Err(e) => {
                    warn!(
                        "URL: Sig fetch fail for ({},{}): {}.",
                        e1_id_ordered.0, e2_id_ordered.0, e
                    );
                    None
                }
            };
        ...
        let distance = if let (Some(lat1), Some(lon1), Some(lat2), Some(lon2)) = (
            entity1_info.latitude,
            entity1_info.longitude,
            entity2_info.latitude,
            entity2_info.longitude,
        ) {
        ...
        if let Some(dist) = distance {
            if dist > MAX_LOCATION_DISTANCE_METERS {
                let slugs = count_matching_slugs(
                    &entity1_info.normalized_data.path_slugs,
                    &entity2_info.normalized_data.path_slugs,
                );
                if slugs < 2 {
                    // Stricter condition for distant pairs
                    final_confidence_for_cache_entry = calculate_url_match_confidence(
                        &entity1_info,
                        &entity2_info,
                        slugs,
                        distance,
                    )
                    .0; // Store actual low score
                    ...
                    if let (Some(s1), Some(s2)) = (&sig1_for_store, &sig2_for_store) {
                        if let Err(e) = store_in_comparison_cache(
                            ...
                            Some(final_confidence_for_cache_entry),
                            None,
                        )
                        .await
                        {
        ...
        let matching_slug_count = count_matching_slugs(
            &entity1_info.normalized_data.path_slugs,
            &entity2_info.normalized_data.path_slugs,
        );
        let (base_confidence, match_values_obj) = calculate_url_match_confidence(
            &entity1_info,
            &entity2_info,
            matching_slug_count,
            distance,
        ...
        if reinforcement_orchestrator_option.is_some() {
            if let Some(rl_cache_svc) = rl_feature_cache.as_ref() {
                ...
                let guard = rl_cache_svc.lock().await;
                needs_rl_feature_extraction = !guard.pair_cache.contains(&key);
            } else {
                needs_rl_feature_extraction = true;
            }
        }

        if needs_rl_feature_extraction && reinforcement_orchestrator_option.is_some() {
            rl_feature_cache_miss_items.push(RlCacheMissInfo {
                entity1_info: entity1_info.clone(),
                entity2_info: entity2_info.clone(), // Store full info
                ordered_e1_id: e1_id_ordered.clone(),
                ordered_e2_id: e2_id_ordered.clone(),
                base_confidence,
                match_values: match_values_obj,
                sig1_at_comp: sig1_for_store.clone(),
                sig2_at_comp: sig2_for_store.clone(),
            ...
            if let Some(ro_arc) = reinforcement_orchestrator_option {
                match apply_rl_tuning_cached(
                    ro_arc,
                    pool,
                    &e1_id_ordered,
                    &e2_id_ordered,
                    base_confidence,
                    rl_feature_cache.as_ref(),
                )
                .await
                {
            ...
            if let (Some(s1), Some(s2)) = (&sig1_for_store, &sig2_for_store) {
                if let Err(e) = store_in_comparison_cache(
                    ...
                    comparison_outcome_for_cache,
                    Some(final_confidence_for_cache_entry),
                    features_json_for_main_cache.as_ref(),
                )
                .await
                {
                    warn!(
                        "URL: Store comp cache fail for ({},{}): {}",
                        e1_id_ordered.0, e2_id_ordered.0, e
                    );
                }
            }
            ...
            pairs_to_create_in_db_batch.push(PairToCreate {
                entity_group_id: new_gid.clone(),
                entity_id_1: e1_id_ordered.clone(),
                entity_id_2: e2_id_ordered.clone(),
                match_values: match_values_obj.clone(),
                final_confidence,
                pre_rl_confidence: base_confidence,
                features_for_decision_log: temp_features_for_decision_log,
            });
            if final_confidence < MODERATE_LOW_SUGGESTION_THRESHOLD {
                suggestions_to_create_in_db_batch.push(create_suggestion_data(
                    &new_gid,
                    &e1_id_ordered,
                    &e2_id_ordered,
                    final_confidence,
    ...
    if !rl_feature_cache_miss_items.is_empty() {
        let feature_results = batch_extract_features_for_url(
            pool,
            &rl_feature_cache_miss_items
                .iter()
                .map(|item| (item.ordered_e1_id.clone(), item.ordered_e2_id.clone()))
                .collect::<Vec<_>>(),
        )
        .await;

        for (i, mut item) in rl_feature_cache_miss_items.into_iter().enumerate() {
            // item is mutable
                    ...
                    Ok(features_vec) => {
                        ...
                        if !features_vec.is_empty() {
                            ...
                            if let Some(rl_cache_svc) = rl_feature_cache.as_ref() {
                                ...
                                let mut guard = rl_cache_svc.lock().await;
                                guard.pair_cache.put(key, features_vec.clone());
                            }
                            if let Some(ro_arc) = reinforcement_orchestrator_option {
                                ...
                                match guard.get_tuned_confidence(
                                    ...
                                    Err(e) => warn!(
                                        "URL: RL tuning (batch) fail for ({},{}): {}.",
                                        item.ordered_e1_id.0, item.ordered_e2_id.0, e
                                    ),
                                }
                            }
                        }
            ...
            if let (Some(s1), Some(s2)) = (&item.sig1_at_comp, &item.sig2_at_comp) {
                ...
                {
                    warn!(
                        "URL: Store comp cache (RL batch) fail for ({},{}): {}",
                        item.ordered_e1_id.0, item.ordered_e2_id.0, e
                    );
                }
            }
            ...
            pairs_to_create_in_db_batch.push(PairToCreate {
                entity_group_id: new_gid.clone(),
                entity_id_1: item.ordered_e1_id.clone(),
                entity_id_2: item.ordered_e2_id.clone(),
                match_values: item.match_values.clone(),
                final_confidence,
                pre_rl_confidence: item.base_confidence,
                features_for_decision_log: temp_features_for_decision_log,
            });
            if final_confidence < MODERATE_LOW_SUGGESTION_THRESHOLD {
                suggestions_to_create_in_db_batch.push(create_suggestion_data(
    ...
    if !pairs_to_create_in_db_batch.is_empty() {
        match batch_create_url_entity_groups(
            pool,
            pairs_to_create_in_db_batch,
            suggestions_to_create_in_db_batch,
            reinforcement_orchestrator_option,
            pipeline_run_id,
        )
        .await
    ...
    Ok((
        domain_comparison_cache_hits,
        domain_rl_feature_extractions,
        domain_rl_feature_failures,
    ))
}
...
async fn batch_create_url_entity_groups(
    ...
    // Returns count of successfully *newly* inserted groups
    ...
    let tx = conn
        .transaction()
        .await
        .context("URL: Start TX batch create")?;

    let entity_group_stmt = tx
        .prepare(INSERT_ENTITY_GROUP_SQL)
        .await
        .context("URL: Prepare eg insert")?;

    let mut successfully_inserted_new_groups_data = Vec::new();

    for pair_item in pairs_to_create {
        let mv_json =
            serde_json::to_value(&pair_item.match_values).context("URL: Serialize mv for eg")?;
        let rows = tx
            .query(
                &entity_group_stmt,
                &[
                    &pair_item.entity_group_id.0,
                    &pair_item.entity_id_1.0,
                    &pair_item.entity_id_2.0,
                    &MatchMethodType::Url.as_str(),
                    &mv_json,
                    &pair_item.final_confidence,
                    &pair_item.pre_rl_confidence,
                ],
            )
            .await
            .context("URL: Insert eg failed")?;

        if let Some(row) = rows.first() {
            // Check if RETURNING id gave us something
            let created_group_id: String = row.get(0); // Assuming 'id' is the first column returned
            if created_group_id == pair_item.entity_group_id.0 {
                // Ensure it's the one we tried to insert
    ...
    if !successfully_inserted_new_groups_data.is_empty() {
        if let Some(ro_arc) = reinforcement_orchestrator_option {
            ...
            let decision_stmt = tx
                .prepare(INSERT_DECISION_SQL)
                .await
                .context("URL: Prepare decision log insert")?;
                ...
                if let Some(features) = &created_pair_data.features_for_decision_log {
                    ...
                    if let Err(e) = tx
                        .execute(
                            &decision_stmt,
                            &[
                                &created_pair_data.entity_group_id.0,
                                &pipeline_run_id,
                                &snapshot_features_json,
                                &MatchMethodType::Url.as_str(),
                                &created_pair_data.pre_rl_confidence,
                                &created_pair_data.final_confidence,
                    ...
                    {
                        warn!(
                            "URL: Failed to log decision for group {}: {}",
                            created_pair_data.entity_group_id.0, e
                        );
                    }
                }
            }
    ...
    if !suggestions_to_create.is_empty() {
        let suggestion_stmt = tx
            ...
            .await
            .context("URL: Prepare suggestion insert")?;
        for sugg_item in suggestions_to_create {
            ...
            if let Err(e) = tx
                .execute(
                    &suggestion_stmt,
                    &[
                        &sugg_item.pipeline_run_id,
                        &sugg_item.action_type,
                        &sugg_item.group_id,
                        &sugg_item.triggering_confidence,
                        &sugg_item.details,
                        &sugg_item.reason_code,
                        &sugg_item.reason_message,
                        &sugg_item.priority,
                        &sugg_item.status,
                    ],
                )
            ...
            {
                warn!(
                    "URL: Insert suggestion failed for group {}: {}",
                    sugg_item.group_id, e
                );
            }
            // }
        }
...
async fn fetch_existing_entity_groups(pool: &PgPool) -> Result<Arc<HashSet<(EntityId, EntityId)>>> {
    ...
    for r in rows {
        ...
        if id1 < id2 {
            pairs.insert((EntityId(id1), EntityId(id2)));
        } else {
            pairs.insert((EntityId(id2), EntityId(id1)));
        }
    }
    Ok(Arc::new(pairs))
}
...
async fn fetch_entities_with_urls_and_locations(pool: &PgPool) -> Result<Vec<EntityUrlInfo>> {
    ...
    let rows = conn
        .query(query, &[])
        .await
        .context("URL: Query entities/URLs")?;
    let mut entities = Vec::with_capacity(rows.len());
    for r in rows {
        let (eid_s, url_s, ename_o, lat_o, lon_o): (
            String,
            String,
            Option<String>,
            Option<f64>,
...
fn apply_business_logic_filtering(
    ...
    for (domain, entities) in map {
        if entities.len() < 2 {
            continue;
        } // Need at least two entities in a domain to form a pair
    ...
    if excluded_count > 0 {
        info!(
            "URL: Excluded {} domains by entity count threshold.",
            excluded_count
        );
    }
    filtered
...
async fn batch_extract_features_for_url(
    ...
    let batch_size = (pairs.len() / num_cpus)
        .max(1)
        .min(RL_FEATURE_EXTRACTION_BATCH_SIZE);
    ...
    for chunk in pairs.chunks(batch_size) {
        ...
        for res_outer in chunk_results {
            match res_outer {
                ...
                Err(join_err) => results.push(Err(anyhow!(
                    "Task join error for RL feature extraction: {}",
                    join_err
                ))),
            }
        }
...
async fn apply_rl_tuning_cached(
    ...
    // Return features for decision logging
    ...
    match features_vec_res {
        Ok(features) => {
            if features.is_empty() {
                return Ok((base_confidence, None));
            } // No features, return base confidence
            let guard = ro_arc.lock().await;
            match guard.get_tuned_confidence(&MatchMethodType::Url, base_confidence, &features) {
                ...
                Err(e) => {
                    warn!(
                        "URL: RL get_tuned_confidence fail for ({},{}): {}. Using base confidence.",
                        e1_id.0, e2_id.0, e
                    );
        ...
        Err(e) => {
            warn!(
                "URL: Feature extraction failed during RL tuning for ({},{}): {}",
                e1_id.0, e2_id.0, e
            );
...
fn create_suggestion_data(
    ...
    let priority = if final_confidence < CRITICALLY_LOW_SUGGESTION_THRESHOLD {
        2
    } else {
        1
    };
    ...
    let (orig_url1, orig_url2, norm_domain, slug_count) =
        if let MatchValues::Url(ref uv) = match_values {
            (
...
fn calculate_url_match_confidence(
    ...
    let (ordered_e1, ordered_e2) = if e1.entity_id.0 <= e2.entity_id.0 {
        (e1, e2)
    } else {
        (e2, e1)
    };
    let full_match = slugs == ordered_e1.normalized_data.path_slugs.len()
        && slugs == ordered_e2.normalized_data.path_slugs.len()
        && !ordered_e1.normalized_data.path_slugs.is_empty();
    let mut conf = if full_match {
    ...
    if let Some(d) = dist_m {
        ...
        if d < 100.0 && slugs > 0 {
            conf = (conf + 0.05).min(0.98);
        }
        // Very close, boost
        else if d > 10000.0 && slugs == 0 {
            conf = (conf - 0.15).max(0.55);
        }
        // Very far, domain only, penalize more
        else if d > MAX_LOCATION_DISTANCE_METERS && slugs <= 1 {
            conf = (conf - 0.08).max(0.60);
        } // Far, few slugs, penalize
    }
    ...
    if slugs == 0 && conf > 0.72 {
        if let (Some(n1_str), Some(n2_str)) = (&ordered_e1.entity_name, &ordered_e2.entity_name) {
            ...
            // One or both names missing, be more conservative
            conf = (conf * 0.92).max(CONFIDENCE_DOMAIN_ONLY - 0.02);
        }
    }
    (
        conf,
        MatchValues::Url(UrlMatchValue {
            original_url1: ordered_e1.original_url.clone(),
            original_url2: ordered_e2.original_url.clone(),
            normalized_shared_domain: ordered_e1.normalized_data.domain.clone(),
            matching_slug_count: slugs,
        }),
    )
}
...
pub fn normalize_url_with_slugs(url_s: &str) -> Option<NormalizedUrlData> {
    let tr = url_s.trim();
    if tr.is_empty() || tr.starts_with("mailto:") || tr.starts_with("tel:") {
        return None;
    }
    let w_scheme = if !tr.contains("://") {
        format!("https://{}", tr)
    } else {
        tr.to_string()
    };
    match StdUrl::parse(&w_scheme) {
        Ok(p_url) => p_url.host_str().and_then(|h| {
            ...
            let dnw = if hl.starts_with("www.") {
                hl.trim_start_matches("www.").to_string()
            } else {
                hl
            };
            if dnw.is_empty() || !dnw.contains('.') || is_ip_address(&dnw) {
                None
            } else {
                Some(NormalizedUrlData {
                    domain: dnw.clone(),
                    path_slugs: p_url.path_segments().map_or_else(Vec::new, |s| {
                        s.filter(|ps| !ps.is_empty() && ps.len() < 50 && !ps.contains('.'))
                            .map(|ps| ps.to_lowercase())
                            .collect()
                    }),
                    original_url: url_s.to_string(),
                    domain_type: categorize_domain(&dnw),
                })
            }
        }),
        Err(_) => {
            ...
            let dom_part = scheme_less
                .split(|c: char| c == '/' || c == '?' || c == '#')
                .next()
                .unwrap_or("")
                .to_lowercase();
            let dnw = if dom_part.starts_with("www.") {
                dom_part.trim_start_matches("www.").to_string()
            } else {
                dom_part
            };
...
pub fn is_ignored_domain(domain: &str) -> bool {
    const IGNORED_SUFFIXES_AND_DOMAINS: &[&str] = &[
        ...
        "vimeo.com",
        "soundcloud.com",
        "github.io", // github.com itself might be valid for orgs
        // Major generic service providers often used for redirects or secondary purposes
        "google.com",
        "apple.com",
        "microsoft.com", // These are too broad, but specific subdomains might be ignored
        // URL Shorteners
        "bit.ly",
        "t.co",
        "goo.gl",
    ...
    IGNORED_SUFFIXES_AND_DOMAINS
        .iter()
        .any(|suf| domain == *suf || domain.ends_with(&format!(".{}", suf)) || *suf == domain)
}

fn categorize_domain(dom: &str) -> DomainType {
    if dom.ends_with(".gov")
        || dom.ends_with(".mil")
        || dom.ends_with(".GOV")
        || dom.ends_with(".MIL")
    {
        DomainType::Government
    } else if dom.ends_with(".edu")
        || dom.contains("k12")
        || dom.contains("school")
        || dom.contains("college")
        || dom.contains("university")
        || dom.ends_with(".EDU")
    {
        DomainType::Education
    } else if dom.ends_with(".org")
        || dom.contains("health")
        || dom.contains("medical")
        || dom.contains("clinic")
        || dom.contains("hospital")
        || dom.contains("foundation")
        || dom.contains("charity")
        || dom.contains("nonprofit")
        || dom.ends_with(".ORG")
    {
        DomainType::Healthcare
    }
    // Broadened to .org common for non-profits
    else if dom.ends_with(".com")
        || dom.ends_with(".net")
        || dom.ends_with(".io")
        || dom.ends_with(".co")
        || dom.ends_with(".biz")
    ...
    else if dom.contains("facebook.")
        || dom.contains("twitter.")
        || dom.contains("instagram.")
        || dom.contains("linkedin.")
        || dom.contains("youtube.")
    {
        DomainType::Social
    } else {
        DomainType::Other
...
fn is_ip_address(dc: &str) -> bool {
    ...
    if dc.split('.').count() == 4 && dc.split('.').all(|p| p.parse::<u8>().is_ok()) {
        return true;
    }
    // Basic IPv6 check: contains colons, typically more than one.
    if dc.contains(':') && dc.matches(':').count() >= 2 {
        return true;
    }
    false
}

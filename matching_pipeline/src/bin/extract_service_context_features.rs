...
use std::{path::Path, sync::{Arc, atomic::{AtomicUsize, Ordering}}, time::Instant};
...
async fn process_service_batch(
    ...
    for service_id in services {
        ...
        let task = tokio::spawn(async move {
            ...
            let _permit = service_semaphore.acquire().await.expect("Semaphore acquisition failed");
            
            let conn = match service_pool.get().await {
                Ok(conn) => conn,
                Err(e) => {
                    warn!("Failed to get DB connection for service {}: {}", service_id.0, e);
                    return Err(anyhow::anyhow!("DB connection error"));
                }
            };
            
            // Get or extract features for this service
            let result = service_feature_extraction::get_stored_service_features(&conn, &service_id).await;
...
async fn main() -> Result<()> {
    ...
    let service_ids: Vec<ServiceId> = service_rows.iter()
    ...
    for (batch_idx, batch) in service_batches.clone().into_iter().enumerate() {
        debug!("Processing batch {}/{}", batch_idx + 1, service_batches.len());
        ...
        if success_count < batch_size {
            warn!("Batch {}/{}: {} of {} services were successfully processed",
                batch_idx + 1, service_batches.len(), success_count, batch_size);
        }
    }
    
    info!("Successfully extracted features for {}/{} services", total_success, total_services);
    ...
    info!("Service context feature extraction completed in {:.2?}", elapsed);

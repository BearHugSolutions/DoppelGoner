...
use dedupe_lib::utils::progress_bars::progress_config::ProgressConfig; // Added for pipeline_run entry
...
async fn main() -> Result<()> {
    ...
    let total_clusters = run_entity_clustering(&pool, &run_id, multi_progress.clone())
        .await
        .context("Entity clustering failed")?;
    ...
    info!("Standalone entity clustering pipeline completed successfully!");
    Ok(())
}

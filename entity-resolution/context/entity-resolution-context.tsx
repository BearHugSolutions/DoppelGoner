...
export function EntityResolutionProvider({
    ...
    for (let i = 0; i < trulyNeedsFetching.length; i += MAX_BULK_FETCH_SIZE) {
        ...
        try {
            ...
            setConnectionData(prev => {
                ...
                chunk.forEach(requestedItem => {
                    const wasReturned = response.some(r => r.edge.id === requestedItem.edgeId);
                    if (!wasReturned) {
  ...
  const loadConnectionDataForLinkPage = useCallback(async (clusterId: string, pageToLoad: number, isPrefetch: boolean = false) => {
    ...
    if (!isPrefetch) {
        ...
        if (endIndex < totalLinks) { // Check if there's a next page to prefetch
  ...
  const initializeLargeClusterConnectionPaging = useCallback(async (clusterId: string) => {
    ...
    if (!viz || visualizationData[clusterId]?.error || !visualizationData[clusterId]?.data?.links) { // Check if links are present
  ...
  const loadSingleConnectionData = useCallback(
    async (edgeId: string): Promise<EntityConnectionDataResponse | null> => {
      ...
      if (
        cached?.data && !cached.loading && !cached.error && cached.lastUpdated &&
      ...
      try {
        ...
        const fetcher = resolutionMode === 'entity' 
        ...
        setConnectionData(prev => ({
            ...prev,
            [edgeId]: { data: response, loading: false, error: null, lastUpdated: Date.now() }
        }));
        
        // After getting connection data, fetch details for the nodes involved.
        if (response) {
            const nodesToLoad: NodeIdentifier[] = [];
            if (response.entity1) nodesToLoad.push({ id: response.entity1.id, nodeType: resolutionMode });
            if (response.entity2) nodesToLoad.push({ id: response.entity2.id, nodeType: resolutionMode });
        ...
        console.error(`Error in loadSingleConnectionData for edge ${edgeId}:`, error);
        setConnectionData(prev => ({
            ...prev,
  ...
  const invalidateVisualizationData = useCallback(
    ...
    [resolutionMode, activelyPagingClusterId, initializeLargeClusterConnectionPaging]
  );
  ...
  const currentVisualizationDataForSelection = useMemo((): EntityVisualizationDataResponse | null => {
  ...
  const selectNextUnreviewedEdge = useCallback(
    (afterEdgeId?: string | null) => {
      ...
      console.log(`Attempting to select next unreviewed edge in cluster ${currentClusterId}, after: ${afterEdgeId || lastReviewedEdgeId || selectedEdgeId || "start"}`);
      ...
      if (!currentVizForSelection?.links || currentVizForSelection.links.length === 0) {
        if (currentClusterId === activelyPagingClusterId) {
            ...
            if (fullViz && fullViz.links.length > (largeClusterConnectionsPage * CONNECTION_PAGE_SIZE)) {
                console.log(`No more unreviewed links on current page ${largeClusterConnectionsPage} of large cluster ${currentClusterId}. User may need to advance page.`);
                return;
            }
        }
        console.log(`No links in current view for cluster ${currentClusterId}. Checking advance.`);
        checkAndAdvanceIfComplete(currentClusterId);
        return;
      }
      ...
      if (referenceEdgeId) {
        ...
        else console.warn(`Ref edge ${referenceEdgeId} not found in current view. Starting from beginning of current view.`);
      }
      ...
      console.log(`No more unreviewed edges in current view of cluster ${currentClusterId}. Checking advance.`);
      setSelectedEdgeIdState(null);
      checkAndAdvanceIfComplete(currentClusterId);
    },
    [
  ...
  const selectNextUnreviewedInCluster = useCallback(() => {
    ...
    if (selectedEdgeId) {
      ...
      if (currentIndexInUnreviewedView !== -1) {
        nextEdgeToSelectId = unreviewedLinksInView[(currentIndexInUnreviewedView + 1) % unreviewedLinksInView.length].id;
      } else {
  ...
  const selectPreviousUnreviewedInCluster = useCallback(() => {
    ...
    if (selectedEdgeId) {
      ...
      if (currentIndexInUnreviewedView !== -1) {
        prevEdgeToSelectId = unreviewedLinksInView[(currentIndexInUnreviewedView - 1 + unreviewedLinksInView.length) % unreviewedLinksInView.length].id;
      } else {
  ...
  const submitEdgeReview = useCallback(
    async (edgeId: string, decision: GroupReviewDecision) => {
      ...
      console.log(`Submitting edge review: ${edgeId}, decision: ${decision}, mode: ${resolutionMode}`);
      ...
      if (!connDataState?.data || !vizDataState?.data) {
        toast({ title: "Data Error", description: "Data not loaded for review. Please wait or refresh.", variant: "destructive" });
        ...
        if (!vizDataState?.data && selectedClusterId && !vizDataState?.loading) {
            ...
            if (connectionCount === undefined || connectionCount === null || connectionCount <= LARGE_CLUSTER_THRESHOLD) {
                ...
                console.error("Missing full visualization data for a large cluster during review submission. Attempting to initialize.");
      ...
      const relevantGroups: EntityGroup[] = connData.entityGroups.filter(
        (group) =>
          (group.entityId1 === (connData.edge as VisualizationEntityEdge).entityId1 &&
            group.entityId2 === (connData.edge as VisualizationEntityEdge).entityId2) ||
          (group.entityId1 === (connData.edge as VisualizationEntityEdge).entityId2 &&
            group.entityId2 === (connData.edge as VisualizationEntityEdge).entityId1)
      );
      ...
      if (!currentEdgeLink) {
        console.error("Critical: Current edge link not found in full visualization data for optimistic update.");
      ...
      setVisualizationData((prevVizData) => {
        ...
        if (targetClusterVizState?.data) {
          const targetClusterViz = JSON.parse(JSON.stringify(targetClusterVizState.data)) as EntityVisualizationDataResponse;

          const linkIndex = targetClusterViz.links.findIndex((l) => l.id === batch.edgeId);
          if (linkIndex !== -1) {
            targetClusterViz.links[linkIndex].status = batch.optimisticEdgeStatus;
          }

          if (targetClusterViz.groups && Array.isArray(targetClusterViz.groups)) {
      ...
      setConnectionData((prevConnData) => {
        ...
        if (targetConnState?.data) {
          let targetConn = JSON.parse(JSON.stringify(targetConnState.data)) as EntityConnectionDataResponse;
  ...
  const processReviewQueue = useCallback(async () => {
    ...
    const batchToProcess = reviewQueue.find(
      (b) => !b.isTerminalFailure && (b.failedOperations.size === 0 || b.attempt < MAX_REVIEW_ATTEMPTS)
    );
    if (!batchToProcess) {
    ...
    for (const op of stillPendingOperations) {
      try {
        const payload = { decision: currentBatch.decision, reviewerId: currentBatch.reviewerId };
    ...
    if (!batchOverallSuccess) {
      ...
      if (currentBatch.attempt >= MAX_REVIEW_ATTEMPTS) {
        ...
        toast({
          ...
          description: (
            <>
              Failed: connection {currentBatch.edgeId.substring(0, 8)}... ({currentBatch.mode}). Error: {currentBatch.error}
        ...
        setVisualizationData((prevVizData) => {
          ...
          if (targetClusterVizState?.data) {
            ...
            if (linkIndex !== -1) {
              targetClusterViz.links[linkIndex].status = currentBatch.originalEdgeStatus;
            }
            if (targetClusterViz.groups && Array.isArray(targetClusterViz.groups)) {
              (targetClusterViz.groups as EntityGroup[]).forEach((group) => {
        ...
        setConnectionData((prevConnData) => {
          ...
          if (targetConnState?.data) {
            ...
            targetConn.edge.confirmedStatus = currentBatch.originalEdgeStatus as string | null;
            targetConn.entityGroups.forEach(group => {
                const op = currentBatch.operations.find(o => o.groupId === group.id);
                if (op) group.confirmedStatus = op.originalGroupStatus;
            });
        ...
        toast({
          ...
          description: `Attempt ${currentBatch.attempt}/${MAX_REVIEW_ATTEMPTS} failed for ${currentBatch.edgeId.substring(0,8)}... Will retry. Error: ${currentBatch.error}`,
          variant: "default",
          duration: 5000,
        });
      }
  ...
  const currentConnectionData = useMemo((): EntityConnectionDataResponse | null => {
  ...
  const edgeSelectionInfo = useMemo((): EdgeSelectionInfo => {
    ...
    const fullVizForStats = selectedClusterId ? visualizationData[selectedClusterId]?.data : null;

    const defaultEdgeInfo: EdgeSelectionInfo = {
        currentEdgeId: selectedEdgeId,
        nextUnreviewedEdgeId: null,
        hasUnreviewedEdges: fullVizForStats?.links?.some((link) => !queries.isEdgeReviewed(link.id)) || false,
        currentEdgeIndex: -1,
        totalEdges: 0,
        totalUnreviewedEdgesInCluster: fullVizForStats?.links?.filter((link) => !queries.isEdgeReviewed(link.id)).length || 0,
    ...
    if (!currentVizForView?.links || currentVizForView.links.length === 0) {
      if (selectedEdgeId && fullVizForStats?.links) {
         const allUnreviewedLinksInCluster = fullVizForStats.links.filter((link) => !queries.isEdgeReviewed(link.id));
    ...
    if (selectedEdgeId) {
        defaultEdgeInfo.currentEdgeIndex = linksInView.findIndex((link) => link.id === selectedEdgeId);
    }
    ...
    if (selectedEdgeId) {
      defaultEdgeInfo.currentUnreviewedEdgeIndexInCluster = allUnreviewedLinksInCluster.findIndex(
        (l) => l.id === selectedEdgeId
      );
    }
    ...
    if (allUnreviewedLinksInCluster.length > 0) {
        if (defaultEdgeInfo.currentUnreviewedEdgeIndexInCluster !== -1 && defaultEdgeInfo.currentUnreviewedEdgeIndexInCluster < allUnreviewedLinksInCluster.length -1) {
            nextUnreviewedEdgeIdInCluster = allUnreviewedLinksInCluster[defaultEdgeInfo.currentUnreviewedEdgeIndexInCluster + 1].id;
        } else if (allUnreviewedLinksInCluster.length > 0 && (defaultEdgeInfo.currentUnreviewedEdgeIndexInCluster === -1 || defaultEdgeInfo.currentUnreviewedEdgeIndexInCluster === allUnreviewedLinksInCluster.length -1)) {
            let searchStartIndexFull = 0;
            if (selectedEdgeId) {
                const currentSelectedFullIndex = allLinksInCluster.findIndex((l) => l.id === selectedEdgeId);
                if (currentSelectedFullIndex !== -1) searchStartIndexFull = currentSelectedFullIndex + 1;
            }
            for (let i = 0; i < allLinksInCluster.length; i++) {
                const linkToTest = allLinksInCluster[(searchStartIndexFull + i) % allLinksInCluster.length];
  ...
  useEffect(() => {
    if (!selectedClusterId && clusters.data.length > 0 && !clusters.loading) {
      ...
      if (firstNonSplitUncompletedCluster) {
        ...
        console.log("All clusters on current page are split or complete. Auto-selecting first for viewing:", firstCluster.id);
  ...
  const actions = useMemo(
    ...
    [
      ...
      triggerRefresh, loadClusters, loadBulkNodeDetails, loadSingleConnectionData,
      invalidateVisualizationData, invalidateConnectionData, clearAllData,
      selectNextUnreviewedEdge, selectNextUnreviewedInCluster,
      selectPreviousUnreviewedInCluster, advanceToNextCluster,
      checkAndAdvanceIfComplete, submitEdgeReview, retryFailedBatch,
  ...
  useEffect(() => {
    if (
      ...
      console.log(`Cluster ${selectedClusterId} selected, (paged) viz data loaded, no edge selected. Evaluating next action using selectNextUnreviewedEdge.`);
  ...
  useEffect(() => {
    if (selectedEdgeId) {
      ...
      if (
        ...
        console.log(`Fetching/Re-fetching connection data for selected edge: ${selectedEdgeId}`);

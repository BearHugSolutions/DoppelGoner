...
export default function GraphVisualizer() {
  ...
  const updateVisualization = useCallback(() => {
    ...
    const linkElements = g
      ...
      .on("mouseover", (event, d) => {
        ...
        d3.select(`#link-${d.id}`)
          .attr("stroke-width", (d.status === "CONFIRMED_MATCH" ? 6 : 
                                 d.status === "CONFIRMED_NON_MATCH" ? 4 : 
                                 1 + d.weight * 5))
      ...
      .on("mouseout", (event, d) => {
        ...
        d3.select(`#link-${d.id}`)
          .attr("stroke-width", (d.status === "CONFIRMED_MATCH" ? 4 :
                                 d.status === "CONFIRMED_NON_MATCH" ? 2 :
                                 1 + d.weight * 3))
          .attr("stroke-opacity", (d.status === "CONFIRMED_NON_MATCH" ? 0.4 :
                                  d.status === "CONFIRMED_MATCH" ? 0.9 :
                                  0.7));
      })
    ...
    const nodeElements = g
      ...
      .append("circle")
      .attr("r", (d) => {
        const hasConfirmedConnections = visibleLinks.some(link => 
          (link.source === d.id || link.target === d.id) && link.status === 'CONFIRMED_MATCH'
        );
        return hasConfirmedConnections ? 10 : 8;
      })
      .attr("fill", (d) => {
        const confirmedConnections = visibleLinks.filter(link => 
          (link.source === d.id || link.target === d.id) && link.status === 'CONFIRMED_MATCH'
        ).length;
        const pendingConnections = visibleLinks.filter(link => 
          (link.source === d.id || link.target === d.id) && link.status === 'PENDING_REVIEW'
        ).length;
      ...
      .html((d) => {
        ...
        return `
          <div style="background: white; padding: 8px; border-radius: 6px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border: 1px solid #e5e7eb; font-size: 11px; max-width: 200px; height: 100%; overflow: hidden;">
            <div style="font-weight: 500; margin-bottom: 4px;">${nodeInfo.name || 'Unknown'}</div>
            <div style="margin-bottom: 2px;"><span style="font-weight: 500;">Source:</span> ${nodeInfo.source_system || 'Unknown'}${nodeInfo.source_id ? ` (${nodeInfo.source_id})` : ''}</div>
            ${contactInfo.address ? `<div style="margin-bottom: 2px;"><span style="font-weight: 500;">Address:</span> ${contactInfo.address}</div>` : ''}
            ${contactInfo.phone ? `<div style="margin-bottom: 2px;"><span style="font-weight: 500;">Phone:</span> ${contactInfo.phone}</div>` : ''}
            ${contactInfo.email ? `<div style="margin-bottom: 2px;"><span style="font-weight: 500;">Email:</span> ${contactInfo.email}</div>` : ''}
            ${contactInfo.url ? `<div style="margin-bottom: 2px;"><span style="font-weight: 500;">URL:</span> <span style="color: #2563eb; word-break: break-all;">${contactInfo.url}</span></div>` : ''}
          </div>
        `;
      });
    ...
    const linkTooltipElements = g
      ...
      .html((d_link: EntityLink) => {
        const linkEntityGroups = entityGroups.filter(group =>
          (group.entity_id_1 === d_link.source && group.entity_id_2 === d_link.target) ||
          (group.entity_id_1 === d_link.target && group.entity_id_2 === d_link.source)
        );
                ...
                <div style="border-radius: 4px; border: 1px solid #e5e7eb; background-color: rgba(243, 244, 246, 0.5); padding: 6px;">
                  ...
                  <div style="font-size: 10px; color: #6b7280; space-y: 2px;">
                    ${group.match_values.type === 'Email' ? `
                      <div>${group.match_values.values.original_email1 || 'N/A'}</div>
                      <div>${group.match_values.values.original_email2 || 'N/A'}</div>
                    ` : ''}
                    ${group.match_values.type === 'Phone' ? `
                      <div>${group.match_values.values.original_phone1 || 'N/A'}</div>
                      <div>${group.match_values.values.original_phone2 || 'N/A'}</div>
                    ` : ''}
                    ${group.match_values.type === 'Address' ? `
                      <div>${group.match_values.values.original_address1 || 'N/A'}</div>
                      <div>${group.match_values.values.original_address2 || 'N/A'}</div>
                    ` : ''}
                    ${group.match_values.type === 'Url' ? `
                      <div style="word-break: break-all;">${group.match_values.values.original_url1 || 'N/A'}</div>
                      <div style="word-break: break-all;">${group.match_values.values.original_url2 || 'N/A'}</div>
                    ` : ''}
                    ${group.match_values.type === 'Name' ? `
                      <div>${group.match_values.values.original_name1 || 'N/A'}</div>
                      <div>${group.match_values.values.original_name2 || 'N/A'}</div>
                    ` : ''}
                  </div>
                </div>
              `).join('')}
        ...
        const clickToReviewHtml = d_link.status === 'PENDING_REVIEW' ? `
    ...
    const simulation = d3
      ...
      .on("tick", () => {
        ...
        nodeElements.attr("cx", (d) => (d as any).x).attr("cy", (d) => (d as any).y)

        labelElements.attr("x", (d) => (d as any).x).attr("y", (d) => (d as any).y)
        ...
        linkTooltipElements
          .attr("x", (d) => ((d.source as any).x + (d.target as any).x) / 2 + 20) 
          .attr("y", (d) => ((d.source as any).y + (d.target as any).y) / 2 - (280 / 2)) 
      })
    ...
    if (selectedEdgeId) {
      const selectedLinkData = visibleLinks.find(l => l.id === selectedEdgeId); // Using visibleLinks
      if (selectedLinkData) {
          d3.select(`#link-${selectedEdgeId}`)
            .attr("stroke-width", (selectedLinkData.status === "CONFIRMED_MATCH" ? 6 :
                                  selectedLinkData.status === "CONFIRMED_NON_MATCH" ? 5 :
  ...
  }, [dimensions, nodes, visibleLinks, entityGroups, selectedEdgeId, actions.setSelectedEdgeId, nodeContactInfo])
  ...
  useEffect(() => {
    ...
    svg.selectAll("foreignObject[id^='tooltip-']")
      .transition()
      .duration(150)
      .style("opacity", 0);
    
    if (hoverNode) {
      svg.select(`#tooltip-${hoverNode}`)
        .transition()
        .duration(150)
        .style("opacity", 1);
    }
  ...
  useEffect(() => {
    ...
    svg.selectAll("foreignObject[id^='link-tooltip-']")
      .transition()
      .duration(150)
      .style("opacity", 0);
    
    if (hoverLink && !selectedEdgeId) {
      svg.select(`#link-tooltip-${hoverLink}`)
        .transition()
        .duration(150)
        .style("opacity", 1);
    }
  ...
  const isLoading = selectedClusterId ? queries.isVisualizationDataLoading(selectedClusterId) : false;
  const error = selectedClusterId ? queries.getVisualizationError(selectedClusterId) : null;
  const hasData = nodes.length > 0; 

  const edgeCounts = useMemo(() => {
    const confirmed = links.filter(l => l.status === 'CONFIRMED_MATCH').length;
    const nonMatch = links.filter(l => l.status === 'CONFIRMED_NON_MATCH').length;
        ...
        <>
          ...
          <div className="absolute top-4 right-4 z-30 flex flex-col gap-1 bg-white/90 backdrop-blur-sm rounded-lg p-2 shadow-lg border">
            <TooltipProvider>
              <div className="flex flex-col gap-1">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button size="sm" variant="ghost" onClick={centerGraph} className="h-8 w-8 p-0" > <RotateCcw className="h-4 w-4" /> </Button>
                  </TooltipTrigger>
                  <TooltipContent side="left"><p>Reset & Center</p></TooltipContent>
                </Tooltip>
              </div>
            </TooltipProvider>
            <div className="text-xs text-center text-gray-500 mt-1 min-w-0"> {Math.round(zoomLevel * 100)}% </div>
          </div>
          ...
          <div className="absolute top-4 left-4 z-30 bg-white/90 backdrop-blur-sm rounded-lg p-3 shadow-lg border">
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex items-center gap-2"> <div className="w-4 h-0.5 bg-green-600"></div> <span>Confirmed Match ({edgeCounts.confirmed})</span> </div>
              <div className="flex items-center gap-2"> <div className="w-4 h-0.5 bg-red-600 opacity-60" style={{borderTop: "1px dashed #dc2626"}}></div> <span>Confirmed Non-Match ({edgeCounts.nonMatch})</span> </div>
              <div className="flex items-center gap-2"> <div className="w-4 h-0.5 bg-blue-500"></div> <span>Pending Review ({edgeCounts.pending})</span> </div>
              <div className="text-xs text-muted-foreground mt-2 pt-2 border-t"> Total: {edgeCounts.total} connections </div>
            </div>
          </div>

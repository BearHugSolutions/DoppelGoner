...
import type {
  ...
  NodeDetailResponse, // Import NodeDetailResponse
...
export default function ConnectionReviewTools() {
  ...
  const currentClusterDetails = selectedClusterId
    ? queries.getClusterById(selectedClusterId)
    : null;
  ...
  if (isLoading && !currentConnectionData) {
    // Show loader only if data is truly absent
    return (
  ...
  if (error && !currentConnectionData) {
    // Show error only if data is truly absent
    return (
    ...
    if (isEntityConnectionData(data, resolutionMode)) {
      ...
      return (
        <Card className="h-full flex flex-col items-center justify-center">
          ...
          <CardContent>
            <p>Could not determine the specific type of connection data.</p>
          </CardContent>
        </Card>
      );
    }
  ...
  // Fetch node details using the new query
    ...
    >
      <div className="flex justify-between items-center flex-shrink-0 pb-2 border-b mb-3">
        <div className="flex items-center gap-2">
          ...
          {isEdgeReviewed &&
            !queueStatus &&
            !isClusterSplit && ( // Don't show if cluster is split
              <Badge
                variant={
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Card>
              ...
              <CardContent className="p-3 pt-0">
                <NodeAttributesDisplay nodeDetails={node1Details} />
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="p-3">
              ...
              <CardContent className="p-3 pt-0">
                <NodeAttributesDisplay nodeDetails={node2Details} />
              </CardContent>
            </Card>
          </div>
                ...
                <CardContent className="p-3 pt-0">
                  ...
                  isEntityConnectionData(
                    currentConnectionData as
                      ...
                      | ServiceConnectionDataResponse, // Cast here after null check
                    resolutionMode
                    ...
                    isServiceConnectionData(
                      currentConnectionData as
                        | EntityConnectionDataResponse
                        | ServiceConnectionDataResponse, // Cast here
                      resolutionMode
                    ) ? ( // Type guard refines
